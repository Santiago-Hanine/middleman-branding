          ...
          {Navlist.map((item, index) => (
            <li className='' key={index}>
              <Link onClick={handleMenu} href={item.link}>
                <a className={pathname === item.link ? 'underline decoration-blue decoration-2 underline-offset-4' : ''}>{item.name}</a> {/* Apply 'text-blue' class if current route matches the item's link */}
              </Link>
            </li>
          ))}
        </ul>
